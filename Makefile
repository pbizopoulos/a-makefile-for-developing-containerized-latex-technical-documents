...
docker: # Generate pdf with results from docker.
	docker build -t reconciler-a-template-for-reproducible-computational-research-papers .
	docker run --rm \
		--user $(shell id -u):$(shell id -g) \
		-w /usr/src/app \
		-e HOME=/usr/src/app/cache \
		-v $(PWD):/usr/src/app \
		$(GPU) reconciler-a-template-for-reproducible-computational-research-papers \
