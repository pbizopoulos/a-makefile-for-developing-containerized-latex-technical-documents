...
The corresponding Dockerfile can be provided by the \textit{author} or generated by tools such as repo2docker~\cite{forde2018reproducible}.
...
\begin{enumerate}
	...
	\item the \textit{author} may verify the reproducibility of the \textit{paper} locally by executing \textit{make test} (or remotely by commiting with a message which contains \textit{[make test]}) which triggers the following:
		\begin{enumerate}
		...
		\end{enumerate}
	\item the \textit{reviewer} may verify the reproducibility of the paper locally (by cloning the repo) or remotely (by inspecting the result of \textit{[make test]}).
\end{enumerate}
...
We provide an open source implementation of the RBs template in which the remote reproducibility verification is done using Github Actions.
The builder VM uses the latest version of Ubuntu and pulls a docker image for building \LaTeX\ from the Github Container Registry.
We demonstrate its use by writing and verifying the reproducibility of this paper.
...
The \textit{author} only needs to edit (or use the `touch' UNIX utility to force) a file from \textit{results code} or \textit{\LaTeX\ code} and the corresponding compilation is automatically triggered with `make'.
...
For demonstration purposes we train and test a VGG11 neural network~\cite{simonyan2014very} with batch normalization in PyTorch~\cite{paszke2019pytorch}, on MNIST~\cite{lecun2010mnist}, FashionMNIST~\cite{xiao2017fashion}, KMNIST~\cite{clanuwat2018deep} and QMNIST~\cite{yadav2019cold}.
...
\begin{figure*}[!t]
	...
	\\
	\subfloat{\includegraphics[width=0.25\textwidth]{results/KMNIST-loss}}
	\subfloat{\includegraphics[width=0.25\textwidth]{results/QMNIST-loss}}
	\caption{Train validation losses.
	Image example created from \textit{results code}.}
	\label{fig:image}
\end{figure*}
