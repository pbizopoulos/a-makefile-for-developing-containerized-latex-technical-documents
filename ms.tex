...
The builder VMs use the latest version of Ubuntu and pull a docker image for building \LaTeX\ from the Github Container Registry.
We demonstrate its use by writing and verifying the reproducibility of this paper and its arXiv version.

Regarding research that is computational costly, the template by default allows the execution and reproducibility verification of a computationally lighter version of the \textit{paper} and provides an argument for executing the whole experiment.
This also helps in faster iterations during research writing and experimentation.
An example of a computationally lighter version in the field of neural networks is setting the number of epochs and number of training samples to a low value.
...
The \textit{author} only needs to edit (or use the `touch' UNIX utility to force) a file from \textit{results code} or \textit{\LaTeX\ code} and the corresponding compilation is automatically triggered.
...
For demonstration purposes we train and test a VGG11 neural network~\cite{simonyan2014very} with batch normalization with PyTorch~\cite{paszke2019pytorch}, on the following datasets:
\begin{itemize}
...
\begin{figure*}[!t]
	...
	\caption{Train validation lossses.
