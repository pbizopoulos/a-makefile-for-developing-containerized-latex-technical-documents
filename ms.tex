...
\begin{abstract}
	Previous works in research reproducibility provided frameworks for writing reproducible research papers, however without considering the extra work that needs to be done by the authors to make their paper reproducible and by the reviewers to verify the paper's reproducibility.
	...
	The previous procedure is completed twice and the paper is verified as reproducible, if and only if the resulted pdf files are identical.
...
Open sourcing the code and the raw data of a computational research paper helps increasing the confidence of its reproducibility but it is not enough to verify it.
...
The proposed template can be applied on computational research papers written in \LaTeX~\cite{lamport1994latex} and the `results code' can be executed within a Docker container or a virtual environment.
