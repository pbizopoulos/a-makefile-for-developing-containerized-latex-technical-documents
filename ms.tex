...
\begin{abstract}
	...
	A Makefile allows the authors to verify the reproducibility of their paper locally.
	The authors may also share the reproducibility status of their paper with reviewers using the following procedure.
	When a code update is pushed on the remote repository, a predetermined number of Virtual Machines (VMs) build and run the code that reproduces the figures, tables and variables (results) of the paper.
	These results are then used during the \LaTeX\ compilation in each VM, to produce corresponding pdf files.
	...
	The two builds in each VM are needed to enable comparison between different builds of the same source code.
	Any non-deterministic stochasticity would produce different pdf files for different builds, thus making the paper non-reproducible.
	We release an open source implementation of this procedure and verify the reproducibility of this paper.
\end{abstract}
...
Open sourcing the code and the raw data of a computational research paper helps in increasing the credibility of the research results but this is not enough to verify its reproducibility.
A common workflow among researchers for verifying computational research papers will push the direction towards creating reproducible research, thus tackling the `reproducibility crisis' problem.

We propose reproducible builds that verify the reproducibility of computational research papers written in \LaTeX.
...
Lastly, another VM compares the pdf files and verifies the research paper as reproducible, if and only if the pdf files are identical bit by bit.

We provide an implementation\footnote{\url{https://github.com/pbizopoulos/reproducible-builds-for-computational-research-papers}} of the Reconciler, that verifies the reproducibility of the arXiv version of this paper\footnote{\url{https://arxiv.org/abs/2005.12660}}.
...
\begin{figure*}[!t]
	...
	In \protect\subref{subfig:workflow} the proposed Reconciler workflow with the $n$ independent branches is depicted one for each VM and the optional branch for verifying arXiv papers, where the branch fetches the results and the \LaTeX\ code from the arXiv servers.
...
\begin{enumerate}
	...
	\item the researcher commits and pushes the changes to the remote repository, which triggers the Reconciler workflow:
		\begin{enumerate}
			...
			\item the verifier VM verifies the research paper as reproducible if and only if the pdf files are the same using bit comparison.
		\end{enumerate}
\end{enumerate}
...
Regarding research that is computational costly or when reproducibility verification is required from the first stages of the research, the Reconciler template provides a debug flag that verifies the reproducibility of a smaller scale of the computational experiment.
...
The Reconciler provides a simple solution in verifying the reproducibility of a computational research paper as a whole, without having to trust a third-party.
...
The Reconciler verifies research reproducibility using the tools that most of the researchers already use (\LaTeX, Docker), outsourcing the procedure to VMs on the cloud.
