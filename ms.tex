...
\begin{itemize}
	\item \textit{document} as a document embedded with computational results,
	\item \textit{results} as the figures, tables and variables that are shown in the \textit{document},
	\item \textit{\LaTeX\ code} as the files containing the main text (\textit{*.tex}) and bibliography (\textit{*.bib}) of the \textit{document},
	...
	\item \textit{author} as the author(s) of the \textit{document} and
	\item \textit{reviewer} as the reviewer(s) of the \textit{document} or anyone interested in verifying the \textit{document} (author(s), journal editors, other researchers).
\end{itemize}
...
\begin{figure}[!t]
	...
	\begin{tikzpicture}[%
			...
			\node[draw, right=4cm of document, fill=white] (pdf) {pdf};
...
\begin{figure}[!t]
	\subfloat{\includegraphics[width=0.25\textwidth]{tmp/MNIST-loss}}
	\subfloat{\includegraphics[width=0.25\textwidth]{tmp/FashionMNIST-loss}}
...
\begin{table}[h]
	...
	\scalebox{0.72}{\input{tmp/metrics.tex}}
\end{table}
